...
['label' => 'Серия подряд',   'value' => $stats['streak'], 'color' => '#F59E0B', 'icon' => '🔥',
 'suffix' => trans_choice('день|дня|дней', (int) $stats['streak'])],
];
@endphp
...
{{-- Приветствие --}}
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Здравствуйте, {{ auth()->user()->name }}!</h1>
</div>
                    ...
                    <div class="flex items-start justify-between gap-2">
                        <div class="min-w-0">
                            ...
                            <div class="mt-1 flex items-center gap-2 flex-wrap">
                                ...
                                <span class="text-xs text-gray-500">{{ $g->deadline->format('d.m.Y') }}</span>
                            </div>
                        </div>
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs whitespace-nowrap {{ $tone['pill'] }}">
                            @if ($daysLeft < 0) просрочено @else {{ $timeLeft }} @endif
                        </span>
                    </div>
                </a>
            </li>
...
(function () {
    ...
    const labels = data.map(d => { const [, m, day] = d.date.split('-'); return `${day}.${m}`; });
    ...
    new Chart(ctx, {
        ...
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: ctx => `${ctx.parsed.y} задач(и)` } },
            },
            scales: {
                y: {
                    beginAtZero: true,
